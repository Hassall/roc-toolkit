...
TEST_GROUP(list_operations2) {
    Object objects[NumObjects];

    List<Object, NoOwnership> list;
};

TEST(list_operations2, empty) {
...
TEST(list_operations2, push_back_one) {
...
TEST(list_operations2, push_back_many) {
...
TEST(list_operations2, push_back_iterate) {
    for (size_t i = 0; i < NumObjects; ++i) {
        list.push_back(objects[i]);
    }
...
TEST(list_operations2, push_front_one) {
...
TEST(list_operations2, push_front_many) {
    for (size_t i = 0; i < NumObjects; ++i) {
        LONGS_EQUAL(i, list.size());
...
TEST(list_operations2, push_front_iterate) {
    for (size_t i = 0; i < NumObjects; ++i) {
        list.push_front(objects[i]);
    }
...
TEST(list_operations2, insert_front) {
    list.push_back(objects[0]);
...
TEST(list_operations2, insert_middle) {
    list.push_back(objects[0]);
...
TEST(list_operations2, remove_front) {
    for (size_t i = 0; i < NumObjects; ++i) {
        list.push_back(objects[i]);
    }
...
TEST(list_operations2, remove_middle) {
    list.push_back(objects[0]);
    list.push_back(objects[1]);
...
TEST(list_operations2, contains) {
